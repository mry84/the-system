...
async function upsertFilm(title: string, year: number) {
  ...
  return prisma.film.create({
    data: { title, year, slug: filmSlug(title, year), posterPath: art.posterPath, tmdbId: art.tmdbId, imdbId: art.imdbId },
  });
}
...
export async function createSystemNight(_: LogState, formData: FormData): Promise<LogState> {
  ...
  let attendeeIds = formData.getAll("attendees").map(String).filter(Boolean);
  for (const name of formData.getAll("newMemberName").map(String).map((n) => n.trim()).filter(Boolean)) {
    const person = await addPendingPersonNamed(name);
    if (person && !attendeeIds.includes(person.id)) attendeeIds.push(person.id);
  }
